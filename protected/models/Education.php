...
class Education extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('educationname, recordstatus', 'required'),
			array('recordstatus', 'numerical', 'integerOnly'=>true),
			array('educationname', 'length', 'max'=>50),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('educationid, educationname, recordstatus', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'employeeeducations' => array(self::HAS_MANY, 'Employeeeducation', 'educationid'),
			'employeefamilys' => array(self::HAS_MANY, 'Employeefamily', 'educationid'),
		);
	}
