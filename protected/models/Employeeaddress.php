 ...
 * @property EmployeeAddresstype $addresstype
 */
class Employeeaddress extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('addressbookid, addresstypeid, cityid, kelurahanid, subdistrictid,
        recordstatus', 'numerical', 'integerOnly'=>true),
			array('addressname', 'length', 'max'=>50),
			array('rt, rw', 'length', 'max'=>5),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('addressid, addressbookid, addresstypeid, addressname, rt, rw, 
        cityid, kelurahanid, subdistrictid,recordstatus', 'safe', 'on'=>'search'),
		);
	}
...
if (isset($_GET['Employeeaddress'])) {
			$criteria->condition='t.addressbookid ='.$_GET['Employeeaddress']['addressbookid'];
		} else {
			$criteria->condition='t.addressbookid=0';
		}
		...
		$criteria->compare('recordstatus',$this->rw);
  ...
  public function searchwstatus()
	{
		...
		$criteria=new CDbCriteria;
    $criteria->with=array('addressbook','addresstype','city','kelurahan','subdistrict');
    $criteria->condition='addressbook.isemployee=1 and t.recordstatus=1';
		$criteria->compare('addressid',$this->addressid);
