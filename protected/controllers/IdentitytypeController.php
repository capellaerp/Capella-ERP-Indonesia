		...
		try
          {
            if($model->save())
            {
              ...
              $this->GetSMessage('cityinsertsuccess');
            }
            else
            {
	  ...
	  $folder=$_SERVER['DOCUMENT_ROOT'].Yii::app()->request->baseUrl.'/upload/';// folder for uploaded files
		...
		{ 
			...
			if (($handle = fopen($file, "r")) !== FALSE) 
			{
				...
				{
					if ($row>0) {
			  $model=Identitytype::model()->findByPk((int)$data[0]);
			  if ($model=== null) {
				$model = new Identitytype();
			  }
			  $model->identitytypeid = (int)$data[0];
			  $model->identitytypename = $data[1];
  ...
  public function actionDownload()
	{
		...
		$this->pdf->title='Identity Type List';
		$this->pdf->AddPage('P');
		...
		$this->pdf->Row(array('Identity Type'));
